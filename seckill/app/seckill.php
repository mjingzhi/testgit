...
class seckill extends common
{
	...
	*/
	protected function order_check_redis($gid){
                ...
                $file = '/Users/a13/phptest/seckill.log';
                $time0 = time();
		$time1 = time();
		$goodsInfo 	= $this->_goodsModel->getGoods($gid);
		$time2 = time();
		$cost1 ="cost1:". ($time2 - $time1).";";
		$content = 'niahaooa';
		file_put_contents($file,$cost1,FILE_APPEND);
 ...
 #               $file = ~/phptest/seckill.txt;

               
                $time3 = time();
		
                $content="订单生成前时间".":".time();
 		//生成订单
		$data             = [];
		$data['order_id'] = $this->_orderModel->buildOrderNo();
		$data['goods_id'] = $goodsInfo['id'];
	...
	#	$content ="" 
	        $time4 = time();
		$cost2 ="cost2:".($time4 - $time3).";";
		file_put_contents($file,$cost2,FILE_APPEND);
		//库存减少
		$time5 = time();
		$gid 	= $goodsInfo['id'];
	...
	#	$this->_error = '购买成功';
	        $time6 = time();
		$cost3 = "cost3:".($time6 - $time5).";";
		file_put_contents($file,$cost3,FILE_APPEND);
		$this->_error = '购买成功';
	        $time7 = time();
		$costtal = "costtal:".($time7 - $time0).":";

		file_put_contents($file,$costtal,FILE_APPEND);
	        return true;
	}
